...
- `662055e` — **Batch 5** (sbatch + root files + duplication diff + hub v0 design): sbatch/ walk; scripts/ vs speciesOT/scripts/ duplication resolved (recommendation captured); root files walked and bucketed; new `docs/hub_design.md` written as first-pass hub v0 architecture.
...
- `10c4a36` — **Batch 9** (cellot/ archive + YAML system docs): git mv 13 stale `speciesot-*.yaml` Layer-1 task templates → `cellot/cellot_gpu/configs/tasks/_archive/speciesot_v1/`; add three-layer YAML system section to scratchpad; add cookbook §12 to `docs/hub_design.md`; also (disk-only) deleted `cellot/_archive/cellot_cpu_clone/` (~2.4 GB freed).
...
- [ ] Update workspace-root `README.md` to mention `docs/conceptual_framework.md`, `docs/hub_design.md`, `REFACTOR_WALKTHROUGH_2026-05-24.md`, and clarify "scripts/ at workspace root is the single source of truth."
