...
def load_autoencoder_model(config, restore=None, device=None, **kwargs):
...
def compute_scgen_shift(model, dataset, labels, device=None):
    model.code_means = dict()

    inputs = next(iter(DataLoader(dataset, batch_size=len(dataset), shuffle=False)))
    if device is not None:
        inputs = inputs.to(device)# input batch needs to be on GPU as well
