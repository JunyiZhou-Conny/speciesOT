...
def load_item_from_save(path, key, default, device=None):
    path = Path(path)
    if not path.exists():
        return default
...
def train_cellot(outdir, config):
    device = resolve_device(config) # read yaml gpu configuration
    ...
    def evaluate():
        target = next(iterator_test_target).to(device) #move each batch from CPU to GPU before the forward path
        ...
        with torch.no_grad():
            ...
            mmd = losses.compute_scalar_mmd(
                target.cpu().numpy(), transport.cpu().numpy()
                # numpy array does not work with gpu, so move back to cpu
            )
    ...
    minmmd = load_item_from_save(cachedir / "model.pt", "minmmd", np.inf, device=device)
    mmd = minmmd
    ...
    for step in ticker:
        ...
        for _ in range(config.training.n_inner_iters):
            source = next(iterator_train_source).to(device).requires_grad_(True)
...
def train_auto_encoder(outdir, config):
    device = resolve_device(config)#same as before
    ...
    best_eval_loss = load_item_from_save(
        cachedir / "model.pt", "best_eval_loss", np.inf, device=device
    )
    ...
    for step in ticker:
        ...
        model.train()
        inputs = next(iterator.train).to(device)# move batch to GPU
        optim.zero_grad()
        ...
        if step % config.training.eval_freq == 0:
            model.eval()
            eval_loss = evaluate(next(iterator.test).to(device))
