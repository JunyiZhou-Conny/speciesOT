...
from cellot.utils.loaders import load, load_data, load_model, resolve_device
...
def load_projectors(aedir, embedding, where):
    ...
    if embedding == "pca":
        ...
        def encode(df):
            inputs = torch.Tensor(df.values).to(ae_device)
            codes = ae.encode(inputs).detach().cpu().numpy()
            return pd.DataFrame(codes, index=df.index)

        def decode(df):
            inputs = torch.Tensor(df.values).to(ae_device)
            recon = ae.decode(inputs).detach().cpu().numpy()
...
def load_conditions(expdir, where, setting, embedding=None):
    ...
    if config.model.name == "identity":
        ...
        device = resolve_device(config)
        model, *_ = load_model(
            config, restore=expdir / "cache" / "model.pt",
        ...
        if (
            ...
            imputed = pd.DataFrame(
                imputed.detach().cpu().numpy(), index=to_pushfwd.adata.obs_names
            )
        else:
