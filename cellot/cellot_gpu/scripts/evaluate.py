...
def main(argv):
    ...
    def iterate_feature_slices():
        ...
        print('[eval] Writing imputed.h5ad...', flush=True)
        imputed.write(cache)
        ...
        if n_markers is not None:
            ...
            total = len(n_markers) * len(all_ncells) * n_reps
            done = 0
            for k in n_markers:
                if k != 'all':
                    feats = markers[:int(k)]
                ...
                for ncells in all_ncells:
                    ...
                    for r in range(n_reps):
                        ...
                        print(f'[eval] {done}/{total} nfeatures={k} ncells={ncells} rep={r+1}/{n_reps}', flush=True)
                        trt = treateddf[feats].sample(ncells)
                        imp = imputeddf[feats].sample(ncells)
                        yield ncells, k, trt, imp

        else:
            total = len(all_ncells) * n_reps
            done = 0
            for ncells in all_ncells:
                if ncells > min(len(treateddf), len(imputeddf)):
                    break
                for r in range(n_reps):
                    ...
                    print(f'[eval] {done}/{total} ncells={ncells} rep={r+1}/{n_reps}', flush=True)
    ...
    print('[eval] Writing evals.csv...', flush=True)
