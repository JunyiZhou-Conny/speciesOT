...
> **Where the scripts live**: the canonical, versioned copies are inside the git repo at `speciesOT/scripts/`. The runtime working tree on the cluster also has a `scripts/` folder at the workspace root (`/n/holylabs/mooney_lab/Lab/junyizhou/speciesOT/scripts/`) — that's where the cron loops and BioMart cache files actually live. When editing a script, edit the git-versioned version first, then `cp` it to the workspace-root location.
