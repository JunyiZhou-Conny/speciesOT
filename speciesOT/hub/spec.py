...
def _datasplit_yaml(spec: ExperimentSpec) -> str:
    """Render the indented `datasplit:` body, including a trailing newline.

    `train_test` only emits keys that `split_cell_data_train_test` / sklearn
    accept. Extra keys (`mode`, `key`, `holdout`, `stratify`) would be
    forwarded into `sklearn.train_test_split` and crash. `toggle_ood` keeps
    the existing block so existing specs are unchanged.
    """
    if spec.datasplit_strategy == "train_test":
        return (
            f"  name: {spec.datasplit_strategy}\n"
            f"  groupby: condition\n"
            f"  random_state: {spec.random_state}\n"
            f"  test_size: {spec.test_size}\n"
        )
    holdout_block = (
        _holdout_yaml(spec.holdout_cell_types)
        if spec.holdout_cell_types
        else "  holdout: ~"
    )
    return (
        f"  name: {spec.datasplit_strategy}\n"
        f"  key: cell_type_ontology_term_id\n"
        f"{holdout_block}\n"
        f"  mode: {spec.mode}\n"
        f"  groupby: condition\n"
        f"  random_state: {spec.random_state}\n"
        f"  test_size: {spec.test_size}\n"
        f"{_stratify_line(spec)}"
    )
...
dataloader:
  ...
  shuffle: true
datasplit:
{_datasplit_yaml(spec)}device: cuda
model:
  beta: 0.0
  dropout: 0.1
  hidden_units:
